...
const Header = () => {
  ...
  return (
    <header className="bg-gray-800 shadow-md fixed w-full text-white ">
      <div className={`flex items-center justify-between mt-2 p-2 mb-2 ${!showMenu ? 'h-14' : 'flex-col h-32'} transition-height duration-300 bg-gray-800`}>
        <div className="flex items-center justify-between w-full">
          <img src="/logo.svg" alt="logo" className="h-14" />
          <img src="name.png" className="md:h-200 flex justify-center items-center" />
          <button 
        ...
        <div className="flex items-center w-full md:w-1/4 md:ml-32">
        ...
        <div className={`${!showMenu ? 'hidden md:flex' : 'flex'} w-full justify-center items-center md:flex ml-0 p-0 `}>
          <ul className="text-size-md flex flex-col md:flex-row md:items-center md:space-x-8 space-y-5 md:space-y-0 mt-5 p-2 md:mt-0 font-bold">
            ...
            <li>
              <a href="/profile" onClick={() => setShowMenu(false)} className="hover:text-black transition-all">
            ...
            <li>
              <a href="/sell" onClick={() => setShowMenu(false)} className="hover:text-black transition-all">
                <button className="p-2 border rounded-lg font-sans font-bold hover:bg-white transition-all hover:text-gray-800">
                  SELL
              ...
              <div className="relative">
                ...
                {showDropdownMenu && (
                  <div className="absolute right-0 mt-2 w-48 bg-white border rounded-md shadow-md">
                    <ul className="py-2">
                      <li>
                        <Link href="/profile" onClick={() => setShowDropdownMenu(false)}
